...
Network::Network(const Network& network)
{
   ...
   // TODO: put something here about casting to GRU etc.
...
/*
   ...
   not a rush through at the beginning.
*/
	...
	while(outputsoff()||!onetime) {
		...
		for(curnode=all_nodes.begin();curnode!=all_nodes.end();++curnode) {
         ...
         //std::cout<<"On node "<<(*curnode)->node_id<<std::endl;
			...
			if ((((*curnode)->type)!=SENSOR)&&(((*curnode)->type)!=GRU)) {
				(*curnode)->activesum=0;
				...
				for(curlink=((*curnode)->incoming).begin();curlink!=((*curnode)->incoming).end();++curlink) {
                  ...
                  //std::cout << (*curlink)->weight << " * " << ((*curlink)->in_node)->get_active_out() << std::endl;
            ...
            for(curlink=((*curnode)->incoming).begin();curlink!=((*curnode)->incoming).end();++curlink) {
					...
					if (!((*curlink)->time_delay)) {
						...
						//Input over a time delayed connection
						link_inputs.push_back(((*curlink)->in_node)->get_active_out_td());
					}
				} //End for over incoming links
		...
		for(curnode=all_nodes.begin();curnode!=all_nodes.end();++curnode) {
            ...
            //NNodeGRU* gru_ptr = dynamic_cast<NNodeGRU*>((*curnode));
               ...
               //std::cout<<"Node "<<(*curnode)->node_id<<" activation count: "<<(*curnode)->activation_count<<std::endl;
				}
...
int Network::max_depth() {
  ...
  for(curoutput=outputs.begin();curoutput!=outputs.end();curoutput++) {
     try {
        ...
        std::cout << "EXCEPTION CAUGHT!! depth: " << depth << std::endl;
        std::cout << "Dumping genome file..." << std::endl;
        print_Genome_tofile(genotype,"broken(loop)_genome");
     }
