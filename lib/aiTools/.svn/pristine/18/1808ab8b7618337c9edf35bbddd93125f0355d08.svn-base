	...
	namespace Math
	{
		...
		struct SparseVector
		{
			...
			void setLength(LengthType l)
			{
				...
				ModifyPairSecond<ModifyWith<std::multiplies<T>, LengthType>> modifier(scaleFactor);

				//TODO: why not for_each??
...
//			QASSERT(beginB == endB);
		...
		{
			ModifyPairSecond<ModifyWith<std::multiplies<T>, U>> modifier(rhs);
		...
		SparseVector<T>& operator/=(SparseVector<T>& lhs, const U& rhs)
		{
			ModifyPairSecond<ModifyWith<std::divides<T>, U>> modifier(rhs);
		...
		{
			ModifyPairSecond<ModifyWith<std::multiplies<T>, U>> modifier(lhs);
		...
		{
			ModifyPairSecond<std::negate<T>> modifier;
		...
		ResultType sparse_average(FwdIterType begin, FwdIterType end, const std::size_t vectorSize)
		{
			...
			while(begin != end)
				result = plus(result, (begin++)->second);

			return divides(result, vectorSize);
		}

		template <typename FwdIterType, typename ResultType = double>
		ResultType sparse_average(FwdIterType begin, FwdIterType end, const std::size_t vectorSize)
		{
			return sparse_average<FwdIterType, ResultType, std::plus, std::divides>(begin, end, vectorSize);
		}
