...
namespace aiTools
{
	///iterates over a container in a random order in O(3n) using std::random_shuffle as random source without modifying the container.
	...
	///iterates over a container in a random order in O(3n) using std::shuffle as random source without modifying the container.
	template <typename ForwardIterator, typename Action, typename RandomNumberGenerator>
	typename std::enable_if<std::is_base_of<std::forward_iterator_tag, typename ForwardIterator::iterator_category>::value, Action>::type
