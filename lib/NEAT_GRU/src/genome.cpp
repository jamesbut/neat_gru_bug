...
Genome::Genome(int id, std::ifstream &iFile) {
	...
	while (!done) {
		...
		//Read in GRU Node
		//TODO: Allow for line of arbitrary size
		else if (strcmp(curword,"nodegru")==0) {
			NNode *newnode;
...
Network *Genome::genesis(int id) {
	...
	for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {
		if((*curnode)->type==GRU) {
			NNodeGRU* gru_ptr = dynamic_cast<NNodeGRU*>((*curnode));
			newnode=new NNodeGRU((*curnode)->node_id,gru_ptr);
		} else {
...
void Genome::print_to_file(std::ofstream &outFile) {
  ...
  for(curnode=nodes.begin();curnode!=nodes.end();++curnode) {
	  if((*curnode)->type==GRU) {
		  NNodeGRU* gru_ptr = dynamic_cast<NNodeGRU*>((*curnode));
		  gru_ptr->print_gru_to_file(outFile);
	  } else {
...
void Genome::mutate_gru_link_weights(double power) {
	...
	for(int i = 0; i < nodes.size(); i++) {
		...
		if(nodes[i]->type == GRU) {
			...
			NNodeGRU* gru_ptr = dynamic_cast<NNodeGRU*>(nodes[i]);
			gru_nodes.push_back(gru_ptr);
...
//TODO: Possibly alter so that a gene cannot disable if it is going
// into a GRU cell
...
bool Genome::mutate_add_gru_node(std::vector<Innovation*> &innovs,int &curnode_id,double &curinnov) {
	...
	//std::cout << "ADDED GRU NODE, ORG #" << genome_id << std::endl;

	//Find random gene to split (other splitting methods were used in the other
	...
	// 	std::cout << "Gene #" << genes[i]->innovation_num<< " " << genes[i]->enable << std::endl;
	// }
...
bool Genome::mutate_add_link(std::vector<Innovation*> &innovs,double &curinnov,int tries) {
	...
	bool done;
	bool do_recur;
	bool loop_recur;	//Whether it is a looped recursive link
	int first_nonsensor;
	...
	if (do_recur) {
		...
		while(trycount<tries) {
			...
			if (loop_recur) {
				...
				//Check that we don't have looped recursion on GRU cell
				if(nodes[nodenum1]->type == GRU) return false;
		...
		while(trycount<tries) {
			...
			// std::cout << "-------------------------" << std::endl;
			// std::cout << "Out node: " << nodep1->node_id << std::endl;
			// std::cout << "In node: " << nodep2->node_id << std::endl;
			// std::cout << "Out node type: " << nodep1->type << std::endl;
			// std::cout << "In node type: " << nodep2->type << std::endl;
			// std::cout << "Is Out node type OUTPUT: " << (nodep1->type==OUTPUT) << std::endl;
			// std::cout << "Is In node type OUTPUT: " << (nodep2->type==OUTPUT) << std::endl;
			// std::cout << "Out gen node label: " << nodep1->gen_node_label << std::endl;
			// std::cout << "In gen node label: " << nodep2->gen_node_label << std::endl;
			...
			while ((thegene!=genes.end()) &&
				...
				if (thegene!=genes.end())
					...
					//ADDED: CONSIDER connections out of outputs recurrent   /* Original code */
					// if (((nodep1->type)==OUTPUT)||
					// 	((nodep2->type)==OUTPUT))
					// 	recurflag=true;
...
double Genome::compatibility(Genome *g) {
	...
	//iterators for moving through the two potential parents' GRU nodes
	std::vector<NNode*>::iterator p1node;
	std::vector<NNode*>::iterator p2node;
	...
	while(!((p1gene==genes.end())&&
		(p2gene==(g->genes).end()))) {
			...
			if (p1gene==genes.end()) {
				...
				if (p1innov==p2innov) {
					...
					//std::cout << (*p1gene)->mutation_num << " - " << (*p2gene)->mutation_num << std::endl;
					//std::cout << ((*p1gene)->lnk)->weight << " " << (*p1gene)->mutation_num << std::endl;
					if (mut_diff<0.0) mut_diff=0.0-mut_diff;		//Take absolute value?
		...
		//TODO: TEST THIS!!
		//Now move through the Nodes of each potential parent
		//to find the GRU units and compare their differences
		int num_matching_gru=0;

		p1node=nodes.begin();
		p2node=(g->nodes).begin();
		while(!((p1node==nodes.end())&&
			(p2node==(g->nodes).end()))) {

				//std::cout << (*p1node)->node_id;

				if (p1node==nodes.end()) {
					//std::cout << "Here1" << std::endl;
					++p2node;
				}
				else if (p2node==(g->nodes).end()) {
					...
					p1innov=(*p1node)->node_id;
					p2innov=(*p2node)->node_id;

					//Only if the nodes have the same node ids and they are both GRU nodes
					if (p1innov==p2innov && (*p2node)->type==GRU && (*p1node)->type==GRU) {
						//std::cout << "Here3" << std::endl;
						num_matching_gru+=1.0;
						//Dynamically cast to GRUNodes
						NNodeGRU* p1_node_ptr = dynamic_cast<NNodeGRU*>((*p1node));
						NNodeGRU* p2_node_ptr = dynamic_cast<NNodeGRU*>((*p2node));
						gru_diff_total+=(p1_node_ptr)->compatibility(*p2_node_ptr);
			...
			double ken_compat = (NEAT::disjoint_coeff*(num_disjoint/1.0)+
				NEAT::excess_coeff*(num_excess/1.0)+
				NEAT::mutdiff_coeff*(mut_diff_total/num_matching));
				//std::cout << "Ken compat: " << ken_compat << std::endl;
