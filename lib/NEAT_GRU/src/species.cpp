	...
	if ((expected_offspring>0)&&
		...
		for (count=0;count<expected_offspring;count++) {		/* James - if this is 0, no offspring created */
			...
			if ((thechamp->super_champ_offspring) > 0) {
				...
				//std::cout << thechamp->super_champ_offspring <<std::endl;
				...
				if ((thechamp->super_champ_offspring) == 1) {
					if (thechamp->pop_champ) {
						//std::cout << "Delta coding thing.." <<std::endl;
						...
						//mom->print_to_file("file_dump/mom_org");
 						//baby->print_to_file("file_dump/baby_org");
					}
				}
		   ...
		   //else if ((!champ_done)&&				//James - I CHANGED THIS
			//	(expected_offspring>5)) {		//James - why does this have to be greater than 5?
					...
					//std::cout << "Mum activation: " << mom->net->outputs[0]->activation << std::endl;
					//std::cout << "Baby activation: " << baby->net->outputs[0]->activation << std::endl;
					...
					//baby->print_to_file("file_dump/baby_org");
					//mom->gnome->print_to_filename("file_dump/mom_gnome");
					//baby->gnome->print_to_filename("file_dump/baby_gnome");
				...
				poolsize==0) {
					...
					if (randfloat()<NEAT::mutate_add_node_prob) {
						...
						mut_struct_baby=true;
					} else if (randfloat()<NEAT::mutate_add_gru_node_prob) {
						new_genome->mutate_add_gru_node(pop->innovations,pop->cur_node_id,pop->cur_innov_num);
						mut_struct_baby=true;
			...
			if (curspecies==(pop->species).end()){
				...
				while((curspecies!=(pop->species).end())&&
					...
					if (found==false) {
					  newspecies=new Species(++(pop->last_species),true);
					  //std::cout<<"CREATING NEW SPECIES "<<pop->last_species<<std::endl;
					  (pop->species).push_back(newspecies);
					  newspecies->add_Organism(baby);  //Add the baby
