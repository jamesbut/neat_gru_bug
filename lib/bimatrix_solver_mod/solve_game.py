...
def process_lrs_output(string_input = None, fpath = 'tmp/out'):
    ...
    for i in range(numberOfEq):
        strat1 = ['{0:F}'.format(float(Fraction(s))) for s in e1[i]]
        strat2 = ['{0:F}'.format(float(Fraction(s))) for s in e2[i]]
        strategies1.append(map(float, strat1))
        strategies2.append(map(float, strat2))
    ...
    return store, index1, index2, strategies1, strategies2

def clique_enumeration(numberOfEq, index1, index2, lib_path):
    ...
    clique_str = lib_path + "/bin/clique <" + lib_path + "/tmp/clique_input.txt >" + lib_path + "/tmp/clique_output.txt"
    os.system(lib_path + "/bin/clique <" + lib_path + "/tmp/clique_input.txt >" + lib_path + "/tmp/clique_output.txt")
    # open clique enumeration output file
    fcout = open(lib_path + '/tmp/clique_output.txt', 'r')
...
def solve_game(lib_path):
    ...
    with open(lib_path + "/tmp/nash_out_1", 'w') as nash_out_1:
        for strat in strategies1:
            for num in strat:
                nash_out_1.write("%s " % num)
            nash_out_1.write("\n")

    with open(lib_path + "/tmp/nash_out_2", 'w') as nash_out_2:
        for strat in strategies2:
            for num in strat:
                nash_out_2.write("%s " % num)
            nash_out_2.write("\n")

    # TODO: tidy up contents of dictionary (e.g. strings to numbers)
    with open(lib_path + '/tmp/out.yaml', 'w') as outfile:
